		...
		async protect(context, { email }) {
			...
			if (result.validity === "valid") return new ArcjetRuleResult({
				...
				return new ArcjetRuleResult({
					...
					state,
					conclusion: "DENY",
					reason: new ArcjetEmailReason({ emailTypes: typedEmailTypes })
				});
			}
		}
	}];
...
function detectPromptInjection(options = {}) {
	...
	return [{
		...
		async protect(context, _details) {
			const ruleId = await hasher.hash(hasher.string("type", type), hasher.uint32("version", version), hasher.string("mode", mode));
			const { fingerprint } = context;
			const [cached, ttl] = await context.cache.get(ruleId, fingerprint);
			if (cached) return new ArcjetRuleResult({
				ruleId,
			...
			return new ArcjetRuleResult({
				...
				state: "NOT_RUN",
				conclusion: "ALLOW",
				reason: new ArcjetPromptInjectionReason({ injectionDetected: false })
			});
		}
	}];
}
